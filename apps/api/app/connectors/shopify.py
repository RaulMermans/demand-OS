"""
ShopifyConnector — disabled stub for Sprint 13.

This connector defines the interface for future Shopify integration.
It is DISABLED by default and will raise NotImplementedError on any live call.

To enable in a future sprint:
  1. Set SHOPIFY_STORE_URL and SHOPIFY_ACCESS_TOKEN in environment.
  2. Implement fetch_* methods using the Shopify Admin REST/GraphQL API.
  3. Add rate limiting, pagination, and incremental sync logic.
  4. Enable via feature flag DEMANDOS_ENABLE_SHOPIFY=true.

Never call live Shopify APIs without explicit user opt-in and credentials.
"""
...
SHOPIFY_REQUIRED_CREDENTIALS = ["SHOPIFY_STORE_URL", "SHOPIFY_ACCESS_TOKEN"]
SHOPIFY_REQUIRED_SCOPES = [
    "read_products", "read_orders", "read_inventory",
    "read_price_rules", "read_fulfillments",
]
SHOPIFY_RATE_LIMIT = "2 requests/second (REST) or leaky-bucket 1000 points/s (GraphQL)"
SHOPIFY_PII_FIELDS = ["customer_email", "customer_name", "shipping_address"]


class ShopifyConnector(BaseCommerceConnector):
    """
    Shopify data source connector — DISABLED.
    ...
    def validate_config(self, config: dict) -> dict:
        ...
        missing = [k for k in SHOPIFY_REQUIRED_CREDENTIALS if k not in config or not config[k]]
        return {
    ...
    def dry_run(self, config: dict) -> dict:
        ...
        return {
            "status": "disabled",
            "message": "ShopifyConnector is disabled. No live API calls made.",
            "would_fetch": [
